...
export const logout = (userId, refreshToken, accessToken) => async (dispatch,getState) => {
    dispatch(isLoading())
    api.post('/user/logout', {token: refreshToken, userId: userId}, getHeaders(accessToken)).then(res => {
        dispatch(success())
        dispatch({
            type: REMOVE_AUTH
        })
    }).catch(err => {
        dispatch(error({for: 'logout', message: err.response.data.error}))
    })
}
